...
- `src/components/PageFrame/PageFrame.astro` - The fixed 0.5px hairline page frame with L-shaped corner marks, plus one ambient corner datum: **bottom-right = current date** (server-rendered fallback, replaced client-side with the viewer's local date). The bottom-left live mouse-coordinate readout was removed at Óscar's request — don't reintroduce it. `pointer-events: none`, `aria-hidden`, hidden in `@media print`; corner data hidden below 768px
